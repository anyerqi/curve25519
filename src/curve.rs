...
use crate::{error::Result, CurveError};
...
impl TryFrom<u8> for KeyType {
    ...
    fn try_from(x: u8) -> Result<Self> {
        match x {
            ...
            t => Err(CurveError::new(napi::Status::Unknown, format!("BadKeyType: {}", t))),
        }
    }
}
...
impl PublicKey {
    ...
    pub fn deserialize(value: &[u8]) -> Result<Self> {
        if value.is_empty() {
            return Err(CurveError::new(napi::Status::Unknown, format!("NoKeyTypeIdentifier")));
        }
        ...
        match key_type {
            KeyType::Djb => {
                ...
                if value.len() < curve25519::PUBLIC_KEY_LENGTH + 1 {
                    return Err(CurveError::new(napi::Status::Unknown, format!("BadKeyLength: {}, {}", KeyType::Djb, value.len())));
                }
    ...
    pub fn from_djb_public_key_bytes(bytes: &[u8]) -> Result<Self> {
        match <[u8; curve25519::PUBLIC_KEY_LENGTH]>::try_from(bytes) {
            Err(_) => Err(CurveError::new(napi::Status::Unknown, format!("BadKeyLength: {}, {}", KeyType::Djb, bytes.len()))),
            Ok(key) => Ok(PublicKey {
                key: PublicKeyData::DjbPublicKey(key),
            }),
        }
...
impl PrivateKey {
    pub fn deserialize(value: &[u8]) -> Result<Self> {
        if value.len() != curve25519::PRIVATE_KEY_LENGTH {
            Err(CurveError::new(napi::Status::Unknown, format!("BadKeyLength: {}, {}", KeyType::Djb, value.len())))
        } else {
